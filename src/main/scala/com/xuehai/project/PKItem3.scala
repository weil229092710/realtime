...
import com.xuehai.project.PKItem1.{getCreateDirectStream, head, kafkaParams, topic, topicSet}
...
/**
  ...
  */
object PKItem3 extends Constants{
	/**
	  * kafka基本配置
	  */
	...
	def sparkMongo(sc: SparkContext): Unit ={
		...
		val docRdd = mongoRdd.map(doc => {
			...
			("%s:%s".format(season, gradeId), userId, gameId, subjectId, star, victory, time,schoolId)
		})

		docRdd.groupBy(_._1) // season:gradeId
				.foreach(x2 => {
			// 一个年级所有数据
			val seasonAndGradeId = x2._1
			val subjectIds = RedisUtil.getString("BIGDATA_KING_PK:%s".format(seasonAndGradeId))
			val userIdList: List[Integer] = x2._2.map(_._2).toList.distinct

			//查询mysql，获得每个学生的schoolId
			// val sql = "SELECT student_id, school_id FROM `fact_student_info` WHERE grade_id=%s".format(seasonAndGradeId.split(":")(1))
//			val sql = "SELECT student_id, school_id FROM ``"
//
//			val results: ResultSet = MysqlUtils.select(sql)
//			val userInfoJson = JSON.parseObject("{}")
//			while(results.next()){
...
//					if(userInfoJson.containsKey(userId.toString)){
//						val value = userInfoJson.getString(userId.toString)
//						userInfoJson.put(userId.toString, value)
//					}else{
//						userInfoJson.put(userId.toString, schoolId)
//					}
//				}
//			}
			...
			if(null==subjectIds || subjectIds.length<1){//如果没有科目ID，则不需要计算
				...
				x2._2.groupBy(_._2).foreach(x3 => {// userId
					try{
						...
						//val schooldId = userInfoJson.getString(userId.toString)
					 val schooldId=x3._2.toList.head._8
				...
				// 存储本赛季、本年级所有的key
				RedisUtil.putString(seasonAndGradeId + ":userIds", allKey.mkString(","))

				// 存储本赛季、本年级所有的gameId
				x2._2.map(x7 => x7._3.toString).sliding(50, 50).toList.distinct.foreach(list => {
					RedisUtil.putmSet(seasonAndGradeId + ":gameIds", list.toList)
				})
